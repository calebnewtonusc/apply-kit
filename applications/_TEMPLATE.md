...
| Info sessions  | Dates and locations                             |
| Admit rate     | If published. It is a rate, not a verdict       |
| Applied before | Yes / no, and the outcome                       |
| **AI policy**  | `allowed` / `disclosed` / `banned` / `unstated` |
...
Prompts verbatim. Never paraphrase. If you do not have the real text yet, write
`NOT YET AVAILABLE` and say what to send.
...
Anything blocking a draft: a decision, a missing prompt, something needing another
person.
