...
units. They drop a class at add/drop. Six weeks later four applications say eighteen
units, every audit passes, and the number is false. Then an interviewer asks what they
are taking this term and the answer does not match the form.

That specific failure is why this command exists. It shipped in a real cycle.
...
- **Never more than six at once.** Take the oldest and the ones that feed the
  application you are actually working on. The rest can wait.
...
```
grep -rn "<old value>" drafts/ applications/ you/
```

**A corrected fact that only gets fixed in `PROFILE.md` is not fixed.** The number is
already sitting in three drafts. Fix all of them in the same turn and say which files
you touched.

If a draft was already submitted with the old value, say so plainly and say what it
costs. Usually it costs one sentence in an interview, said first rather than after
being caught. Do not let them find that out in the room.
